...
def main():
    ...
    print(
        f"[info] {len(corpus)} articles -> {len(chunks)} chunks "
        f"(threshold={threshold} chars, dedupe_repealed={dedupe})",
        file=sys.stderr,
    )
    print(
        f"[info] {split_chunks} paragraph-split chunks, {dedup_chunks} deduped-range chunks, "
        f"{en_whole_chunks} separate whole-English chunks (for articles where AR/EN "
        f"paragraph counts didn't align)",
        file=sys.stderr,
    )
    print(
        f"[info] sum(len(article_numbers)) across all chunks = {total_articles_covered} "
        f"(expected to exceed {len(corpus)} when articles are paragraph-split -- "
        f"the real check is set-based, below)",
        file=sys.stderr,
    )
